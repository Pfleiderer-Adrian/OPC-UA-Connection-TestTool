    ...
    class UAClient
    {
        ...
        public void ReadNodes()
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Read Value = {0} , StatusCode = {1}", result.Value.ToString(), result.StatusCode);
                }
                #endregion
                ...
                Node a = m_session.ReadNode(Variables.Server_NamespaceArray);
        ...
        public void readNode(NodeId id)
        {
            int i = 0;
            ReadValueIdCollection nodesToRead = new ReadValueIdCollection();
            DataValue node = m_session.ReadValue(id);
            
            
            while (i < 99)
            {
                ReadValueId a = new ReadValueId();
                a.NodeId = id;
                a.AttributeId = (uint)i;
                i++;
                nodesToRead.Add(a);
            }
            ...
            foreach (DataValue result in resultsValues)
            {
                ...
                {
                    Console.WriteLine("{0}      Read Value = {1} , StatusCode = {2}", i, result.ToString(), result.Value.ToString());
                }
                i++;
            }
        ...
        public ReferenceDescriptionCollection Browse(NodeId nodeToBrowse)
        {
            ...
            try
            {
                Node a = m_session.ReadNode(nodeToBrowse);
                if (a.NodeClass == NodeClass.Variable)
                {
                    this.readNode(nodeToBrowse);
                    ReferenceDescriptionCollection browseResults2 = new ReferenceDescriptionCollection();
                    return browseResults2;
                }
                ...
                foreach (ReferenceDescription result in browseResults)
                {
                    ...
                    Console.WriteLine("{0}     DisplayName = {1}, NodeClass = {2}, NodeID = {3}", i, result.DisplayName.Text, result.NodeClass, result.NodeId);
                    i++;
                }
